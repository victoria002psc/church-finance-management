...
export const Level4App: React.FC<Level4AppProps> = ({
  ...
  const [sourceBalances] = useState([
    { id: 'src-l3a', l3Name: 'Rev. Dr. Thomas Vance (Region A Treasury)', amount: 12000, lastDisbursed: '2026-08-10' },
    { id: 'src-l3b', l3Name: 'Rev. Sarah Jenkins (Central Mission Fund)', amount: 6500, lastDisbursed: '2026-08-14' },
  ]);
  ...
  const [requests, setRequests] = useState<MoneyRequest[]>([
    ...
    },
    {
      ...
      status: 'APPROVED', // Approved but money not yet given
  ...
  return (
    <div id="level4-app-root" className="min-h-screen bg-[#F7F5F0] text-[#171717] flex flex-col font-sans">
      ...
      {/* HEADER BAR */}
      <header className="border-b border-[#30203D] bg-[#24152F] px-4 sm:px-8 py-3.5 flex items-center justify-between sticky top-0 z-40 shadow-md">
        <div className="flex items-center space-x-3">
          <div className="w-9 h-9 rounded-xl bg-[#30203D] border border-[#D4AF37]/40 text-[#D4AF37] flex items-center justify-center font-bold shadow-xs">
            <Users className="w-5 h-5 text-[#D4AF37]" />
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <span className="font-bold text-white text-sm tracking-tight">Parish Field Operations Workspace</span>
              <span className="px-2 py-0.5 text-[10px] font-bold bg-[#D4AF37] text-[#24152F] rounded-full">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="px-3.5 py-2 bg-[#009E68] hover:bg-[#009E68]/90 text-white font-bold text-xs rounded-xl shadow-md flex items-center space-x-1.5 transition-all cursor-pointer"
          >
      ...
      <div className="flex-1 flex overflow-hidden w-full">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 min-w-0 overflow-y-auto bg-[#F7F5F0] space-y-6">
        
        {/* VIEW 1: WALLET & WORKSPACE OVERVIEW */}
        {activeTab === 'workspace' && (
          <div className="space-y-6 animate-in fade-in duration-150">
            {/* SECTION 1 — FINANCIAL METRICS SUMMARY */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-bold text-[#5F6368] uppercase tracking-wider">Available Funds</div>
                  <button
                ...
                <div className="text-2xl sm:text-3xl font-extrabold text-[#009E68] mt-2 font-mono">
                  ₹{myAvailableBalance.toLocaleString('en-IN')}
                </div>
                <div className="text-[11px] text-[#5F6368] mt-2 font-medium">
                  Current wallet balance for parish project execution
                </div>
              </div>

              <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-5 shadow-xs">
                <div className="text-xs font-bold text-[#5F6368] uppercase tracking-wider">Total Received</div>
                <div className="text-2xl sm:text-3xl font-extrabold text-[#009E68] mt-2 font-mono">
                  ₹{myTotalReceived.toLocaleString('en-IN')}
                </div>
                <div className="text-[11px] text-[#5F6368] mt-2 font-medium">
                  Total disbursed by Supervising L3 Overseers
                </div>
              </div>

              <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-5 shadow-xs">
                <div className="text-xs font-bold text-[#5F6368] uppercase tracking-wider">Total Spent</div>
                <div className="text-2xl sm:text-3xl font-extrabold text-[#171717] mt-2 font-mono">
                  ₹{myTotalSpent.toLocaleString('en-IN')}
                </div>
                <div className="text-[11px] text-[#5F6368] mt-2 font-medium">
                  Vouched & submitted parish expenses
                </div>
              </div>
            </div>
            ...
            {/* SECTION 3 — QUICK WORKER ACTIONS */}
            <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-6 shadow-xs space-y-4">
              <h3 className="text-sm font-bold text-[#171717] tracking-tight">Quick Worker Actions</h3>
              
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <button
                  onClick={() => setShowRequestModal(true)}
                  className="p-4 bg-[#2563EB]/10 hover:bg-[#2563EB]/20 border border-[#2563EB]/30 rounded-2xl text-left space-y-2 transition-all cursor-pointer group"
                >
                  <div className="w-10 h-10 rounded-xl bg-[#2563EB] text-white flex items-center justify-center font-bold shadow-xs group-hover:scale-105 transition-transform">
                    <Send className="w-5 h-5" />
                  </div>
                  <div className="font-bold text-xs text-[#171717]">Request Money</div>
                  <div className="text-[11px] text-[#5F6368] font-medium">Send funding request to L3 supervisor</div>
                </button>

                <button
                  onClick={() => setShowExpenseModal(true)}
                  className="p-4 bg-[#009E68]/10 hover:bg-[#009E68]/20 border border-[#009E68]/30 rounded-2xl text-left space-y-2 transition-all cursor-pointer group"
                >
                  <div className="w-10 h-10 rounded-xl bg-[#009E68] text-white flex items-center justify-center font-bold shadow-xs group-hover:scale-105 transition-transform">
                    <Receipt className="w-5 h-5" />
                  </div>
                  <div className="font-bold text-xs text-[#171717]">Record Expense</div>
                  <div className="text-[11px] text-[#5F6368] font-medium">Log parish project expenditure</div>
                </button>

                <button
                  onClick={() => setShowExpenseModal(true)}
                  className="p-4 bg-[#F59E0B]/10 hover:bg-[#F59E0B]/20 border border-[#F59E0B]/30 rounded-2xl text-left space-y-2 transition-all cursor-pointer group"
                >
                  <div className="w-10 h-10 rounded-xl bg-[#F59E0B] text-[#24152F] flex items-center justify-center font-bold shadow-xs group-hover:scale-105 transition-transform">
                    <Upload className="w-5 h-5" />
                  </div>
                  <div className="font-bold text-xs text-[#171717]">Upload Receipt</div>
                  <div className="text-[11px] text-[#5F6368] font-medium">Scan bill & run OCR review</div>
                </button>

                <button
                  onClick={() => setActiveTab('sources')}
                  className="p-4 bg-[#24152F]/10 hover:bg-[#24152F]/20 border border-[#24152F]/30 rounded-2xl text-left space-y-2 transition-all cursor-pointer group"
                >
                  <div className="w-10 h-10 rounded-xl bg-[#24152F] text-white flex items-center justify-center font-bold shadow-xs group-hover:scale-105 transition-transform">
                    <Layers className="w-5 h-5 text-[#D4AF37]" />
                  </div>
                  <div className="font-bold text-xs text-[#171717]">My Sources</div>
                  <div className="text-[11px] text-[#5F6368] font-medium">View source-wise fund allocations</div>
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'requests' && (
          <div className="space-y-6 animate-in fade-in duration-150">
            <div className="flex items-center justify-between bg-[#FFFDF8] border border-[#E7E2D8] p-5 rounded-2xl shadow-xs">
              <div>
                <h3 className="font-bold text-base text-[#171717]">Money Requests (Sent to L3 Supervisor)</h3>
                <p className="text-xs text-[#5F6368]">Submit and track your funding requests sent to supervising overseers</p>
              </div>
              <button
                onClick={() => setShowRequestModal(true)}
                className="px-4 py-2 bg-[#D4AF37] hover:bg-[#F4E7B5] text-[#24152F] font-bold text-xs rounded-xl shadow-md flex items-center space-x-1.5 cursor-pointer"
              >
            ...
            <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-6 shadow-xs space-y-4">
              <h4 className="text-xs font-bold text-[#5F6368] uppercase tracking-wider">Active & Recent Requests</h4>
              <div className="space-y-3">
                {requests.map((req) => (
                  <div key={req.id} className="p-4 bg-[#F7F5F0] border border-[#E7E2D8] rounded-xl space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div>
                        <div className="font-bold text-sm text-[#171717]">{req.purpose}</div>
                        <div className="text-xs text-[#5F6368]">
                          Target Overseer: <strong>{req.recipientL3Name || 'Level 3 Overseer'}</strong> &bull; {req.categoryName || 'Field Operations'} &bull; {req.date}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-extrabold text-base text-[#171717] font-mono">₹{req.amount.toLocaleString('en-IN')}</div>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-[#E7E2D8] flex items-center justify-between text-[11px]">
                      ...
                      <div className={`flex items-center space-x-1 font-bold ${
                        ...
                        {req.status === 'APPROVED' || req.status === 'MONEY_GIVEN' ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
                      ...
                      <div className={`flex items-center space-x-1 font-bold ${
                        ...
                        {req.status === 'MONEY_GIVEN' ? <CheckCircle2 className="w-3.5 h-3.5 text-[#009E68]" /> : <Clock className="w-3.5 h-3.5" />}
                        <span>3. Money Given</span>
                      </div>
                      <button
                        onClick={() => setSelectedRequestDrawer(req)}
                        className="ml-2 px-2.5 py-1 bg-[#FFFDF8] border border-[#E7E2D8] hover:bg-[#F7F5F0] rounded-lg text-[10px] font-bold text-[#171717] cursor-pointer inline-flex items-center space-x-1"
                      >
        ...
        {activeTab === 'expenses' && (
          <div className="space-y-6 animate-in fade-in duration-150">
            <div className="flex items-center justify-between bg-[#FFFDF8] border border-[#E7E2D8] p-5 rounded-2xl shadow-xs">
              <div>
                <h3 className="font-bold text-base text-[#171717]">Record Expense & Upload Receipts</h3>
                <p className="text-xs text-[#5F6368]">Log parish expenditures, attach bill vouchers, and track overseer review</p>
              </div>
              <button
                onClick={() => setShowExpenseModal(true)}
                className="px-4 py-2 bg-[#009E68] hover:bg-[#009E68]/90 text-white font-bold text-xs rounded-xl shadow-md flex items-center space-x-1.5 cursor-pointer"
              >
                <PlusCircle className="w-4 h-4" />
                <span>+ Record Expense</span>
              </button>
            ...
            <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-6 shadow-xs space-y-4">
              <h4 className="text-xs font-bold text-[#5F6368] uppercase tracking-wider">Submitted Expense Vouchers</h4>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-[#E7E2D8] text-[#5F6368] font-semibold uppercase text-[10px] tracking-wider">
                      <th className="py-2.5 px-3">Date</th>
                      <th className="py-2.5 px-3">Purpose</th>
                      <th className="py-2.5 px-3">Category</th>
                      <th className="py-2.5 px-3 text-right">Amount</th>
                      <th className="py-2.5 px-3 text-center">OCR & Document</th>
                      <th className="py-2.5 px-3 text-center">Overseer Review</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#E7E2D8]">
                    {expenses.map((exp) => (
                      <tr key={exp.id} className="hover:bg-[#F7F5F0] transition-colors">
                        <td className="py-3 px-3 text-[#5F6368] font-mono">{exp.date}</td>
                        <td className="py-3 px-3 font-bold text-[#171717]">{exp.purpose}</td>
                        <td className="py-3 px-3 text-[#5F6368]">{exp.categoryName}</td>
                        <td className="py-3 px-3 text-right font-mono font-bold text-[#171717]">
                          ₹{exp.amount.toLocaleString('en-IN')}
                        </td>
                        <td className="py-3 px-3 text-center">
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#2563EB]/10 text-[#2563EB] border border-[#2563EB]/30">
                            OCR Parsed
                          </span>
                        </td>
                        <td className="py-3 px-3 text-center">
        ...
        {activeTab === 'timeline' && (
          <div className="space-y-6 animate-in fade-in duration-150">
            <div className="bg-[#FFFDF8] border border-[#E7E2D8] p-5 rounded-2xl shadow-xs">
              <h3 className="font-bold text-base text-[#171717]">Money Status & Audit Log</h3>
              <p className="text-xs text-[#5F6368]">Complete chronological status tracking of all requests, disbursements, and expenses</p>
            </div>

            <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-6 shadow-xs space-y-4">
              <div className="space-y-4">
                {requests.map(req => (
                  <div key={`tl-${req.id}`} className="p-4 bg-[#F7F5F0] border border-[#E7E2D8] rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                ...
                {expenses.map(exp => (
                  <div key={`tl-exp-${exp.id}`} className="p-4 bg-[#F7F5F0] border border-[#E7E2D8] rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
        ...
        {activeTab === 'sources' && (
          <div className="space-y-6 animate-in fade-in duration-150">
            <div className="bg-[#FFFDF8] border border-[#E7E2D8] p-5 rounded-2xl shadow-xs">
              <h3 className="font-bold text-base text-[#171717]">My Sources (Fund Provider Allocations)</h3>
              <p className="text-xs text-[#5F6368]">Detailed breakdown of available balances categorized by supervising Level 3 source</p>
            </div>

            <div className="bg-[#FFFDF8] border border-[#E7E2D8] rounded-2xl p-6 shadow-xs space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {sourceBalances.map(src => (
                  <div key={src.id} className="p-5 bg-[#F7F5F0] border border-[#E7E2D8] rounded-xl space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[#24152F] text-[#F4E7B5]">
                          LEVEL 3 PROVIDER
                        </span>
                        <h4 className="font-bold text-sm text-[#171717] mt-1.5">{src.l3Name}</h4>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] uppercase font-bold text-[#5F6368]">Available</div>
                        <div className="text-lg font-extrabold text-[#009E68] font-mono">₹{src.amount.toLocaleString('en-IN')}</div>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-[#E7E2D8] text-[11px] text-[#5F6368]">
                      Last Fund Disbursement: {src.lastDisbursed}
                    </div>
                  </div>
                ))}
              </div>

              <div className="p-4 bg-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-xl text-xs text-[#171717] font-medium leading-relaxed">
                <strong>Deterministic Allocation Rule:</strong> When parish field expenses are recorded, funds are automatically deducted from the higher-balance Level 3 provider source first. Source-level balances remain strictly isolated and auditable.
              </div>
            </div>
          </div>
        )}
